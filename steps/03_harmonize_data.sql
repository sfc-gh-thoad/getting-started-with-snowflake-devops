...
create or replace view silver.flight_emissions as select departure_airport, arrival_airport, avg(estimated_co2_total_tonnes / seats) * 1000 as co2_emissions_kg_per_person
...
create or replace view silver.flight_punctuality as select departure_iata_airport_code, arrival_iata_airport_code, count(case when arrival_actual_ingate_timeliness IN ('OnTime', 'Early') THEN 1 END) / COUNT(*) * 100 as punctual_pct
...
create or replace view silver.flights_from_home as 
  ...
  join silver.flight_punctuality on departure_airport = departure_iata_airport_code and arrival_airport = arrival_iata_airport_code
...
create or replace view silver.weather_forecast as select postal_code, avg(avg_temperature_air_2m_f) avg_temperature_air_f, avg(avg_humidity_relative_2m_pct) avg_relative_humidity_pct, avg(avg_cloud_cover_tot_pct) avg_cloud_cover_pct, avg(probability_of_precipitation_pct) precipitation_probability_pct
...
create or replace view silver.major_us_cities as select geo.geo_id, geo.geo_name, max(ts.value) total_population
...
create or replace view silver.zip_codes_in_city as select city.geo_id city_geo_id, city.geo_name city_geo_name, city.related_geo_id zip_geo_id, city.related_geo_name zip_geo_name
...
create or replace view silver.attractions as select
    city.geo_id,
    city.geo_name,
